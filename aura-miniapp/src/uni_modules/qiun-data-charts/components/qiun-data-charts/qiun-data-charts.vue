...
function resolveUCharts(mod) {
  // 兼容不同打包产物：ESM default / CJS / 命名空间导出
  return (mod && (mod.default || mod.uCharts)) || mod;
}

// 微信小程序端打包后，uCharts 常以 CommonJS 导出“函数本体”，用 require 取值最稳
const uChartsMp = require('../../js_sdk/u-charts/u-charts.js');
const UChartsMpCtor = resolveUCharts(uChartsMp);
...
export default {
  ...
  mixins: [typeof uniCloud !== 'undefined' && uniCloud.mixinDatacom ? uniCloud.mixinDatacom : {}],
  props: {
    type: {
      type: String,
      default: null
  ...
  methods: {
    ...
    _newChart(cid) {
      ...
      this.showchart = true;
      cfu.instance[cid] = new UChartsMpCtor(cfu.option[cid]);
...
const UChartsRDCtor = resolveUCharts(uChartsRD);
...
export default {
  ...
  methods: {
    ...
    newUChart() {
      let cid = this.rid
      cfu.instance[cid] = new UChartsRDCtor(cfu.option[cid])
      cfu.instance[cid].addEventListener('renderComplete', () => {
        that[cid].callMethod('emitMsg',{name:"complete",params:{type:"complete",complete:true,id:cid}})
        cfu.instance[cid].delEventListener('renderComplete')
      });
